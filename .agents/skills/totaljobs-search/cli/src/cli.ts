#!/usr/bin/env bun
// Self-contained CLI for searching jobs on totaljobs.com's public job pages.
...
const HELP = `totaljobs-cli — search jobs on totaljobs.com (UK)
...
SEARCH FLAGS
  ...
  --location, -l <text>   UK location, e.g. "London", "Manchester", "Glasgow".
...
EXAMPLES
  ...
  bun run src/cli.ts search -q "data analyst" -l "Manchester" --jobage 14 --format table
  bun run src/cli.ts search -q "data" --graduate --format table
  bun run src/cli.ts detail 107797816 --format plain

Uses totaljobs.com public pages. Keep volume low.
`
