...
`/scrape` discovers every portal skill under `.agents/skills/*/SKILL.md` and runs its CLI first. This UK fork ships `reed-search` (reed.co.uk — the flagship UK board; searches `config/uk-cities.json` when no `--location` is given), plus the country-agnostic `linkedin-search` and `freehire-search`. Any skill you add with `/add-portal` is included the same way. You do **not** need a matching `site:` line below for those CLIs to run.
